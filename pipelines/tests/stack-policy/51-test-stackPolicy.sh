...
cat <<- "EOF" > /workspace/$gitsource/pipelines/tests/stack-policy/kubectl_stack.txt
{
  ...
  "status": {
    "summary": "[ 0.2.26: error, 0.2.25: active ]",
    "versions": [
      {
        "images": [
          {
            "digest": {
              "message": "Unable to retrieve stack activation digest for image: docker.io/appsody/java-openliberty:0.2.11. Associated stack: java-openliberty 0.2.11. Error: UNAUTHORIZED: authentication required; [map[Action:pull Class: Name:docker.io/appsody/java-openliberty Type:repository]]"
            },
            "id": "Eclipse MicroProfile®",
            "image": "docker.io/kabanerobeta/java-microprofile"
          }
...
stack: kabanerobeta/java-microprofile:0.2.26
EOF
